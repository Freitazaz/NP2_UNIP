...
void menu_aluno(const char *email_usuario) {
    ...
    trim(opcao);
        ...
        IA_Config cfg;
        ia_init(&cfg, "http://127.0.0.1:5000", 5);
        char buf[8192];
        char query[256];
        printf("Digite sua consulta: ");
        if (!fgets(query, sizeof(query), stdin)) continue;
        query[strcspn(query, " ")] = 0;
        if (ia_search(&cfg, query, 5, buf, sizeof(buf)) == 0) {
            printf("Resultados: %s", buf);
        } else {
            printf("Erro ao consultar IA.");
        }
    }
